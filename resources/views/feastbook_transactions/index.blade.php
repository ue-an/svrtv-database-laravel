...
@section('content')
    <div class="bg-light p-5 rounded">
        ...
        <br>
        <h2>Manage Feast Book Transactions</h2>
        <a href="/feastbook-transactions/create" class="btn btn-primary btn-sm float-end">Add Feast Book Transaction</a>
            <table class="table table-striped table-bordered dt-responsive" style="width:100%" id="fbtransactionList">
             <thead>
                 <tr>
                     {{-- <th scope="col">#</th> --}}
                     <th scope="col" style="width: 90px">Transaction ID</th>
             ...
             <tbody>
                 @foreach ($fbtransactions as $fbtran)
                     <tr>
                         <th scope="row"> {{$fbtran->feastbook_id}} </th>
                         <th scope="row"> {{$fbtran->order_id}} </th>
                         <th scope="row"> {{$fbtran->product_id}} </th>
                         <th scope="row"> {{$fbtran->user_id}} </th>
                         <th scope="row"> {{$fbtran->quantity}} </th>
                         <td style="padding-left: 3rem">
                             <a style="padding-left: 21px; padding-right: 21px" href="/feastbook-transactions/{{$fbtran->feastbook_id}}/edit" class="btn btn-primary"><i class="fas fa-pencil-alt"></i></a>
                             <form method="POST" action="/feastbook-transactions/{{ $fbtran->feastbook_id }}">
                                 @csrf
                                 {{ method_field('DELETE') }}
